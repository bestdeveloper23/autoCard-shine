...
function MenubarLogin(editor) {
    ...
    // Initialize the Firebase AuthPage
    const authPage = new AuthPage(editor);
    ...
    LogoutButton.dom.addEventListener('click', async () => {
        await authPage.handleLogout();
    });

    // Initialize the UI based on current login status
    updateSigninStatus();

    return container;
}

// This function is compatible with your existing localStorage checks
function updateSigninStatus() {
    const loginStatus = localStorage.getItem('loginStatus');
    const isSignedIn = loginStatus === 'true';
    ...
    if (loginButton && logoutButton && loginStatusLabel) {
        if (isSignedIn) {
            ...
            if (userEmail) {
                loginStatusLabel.textContent = userEmail;
            }
        } else {
...
// Initialize on page load and listen for storage changes
document.addEventListener('DOMContentLoaded', () => {
    updateSigninStatus();
});

// Listen for localStorage changes to update UI
window.addEventListener('storage', updateSigninStatus);

// Also listen for custom events in case of same-page auth changes
window.addEventListener('authStateChanged', updateSigninStatus);
