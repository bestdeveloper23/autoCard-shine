...
class VRMLLoader extends Loader {
	...
	parse(data, path) {
		...
		function SceneTraverse(object, callback) {

			callback(object);
			const children = object?.children;
			if (children && children.length === 0 || children === undefined) return

			for (let i = 0, l = children.length; i < l; i++) {

				SceneTraverse(children[i], callback);
		...
		if (scene instanceof THREE.Scene) {
			...
			const negativeParticleGroup = new Group();
			const positiveParticleGroup = new Group();
			const neutralParticleGroup = new Group();
			negativeParticleGroup.name = 'Negative Particle';
			positiveParticleGroup.name = 'Positive Particle';
			neutralParticleGroup.name = 'Neutral Particle';
				...
				if (node.isLineSegments) {
					...
					if (material && material.color) {
						...
						if (color.r === 1 && color.g === 0 && color.b === 0) {
							nodesToRemove.push({ node, group: negativeParticleGroup });
						} else if (color.r === 0 && color.g === 1 && color.b === 0) {
							nodesToRemove.push({ node, group: neutralParticleGroup });
						} else if (color.r === 0 && color.g === 0 && color.b === 1) {
							nodesToRemove.push({ node, group: positiveParticleGroup });
						}
					}
				}
			});

			// Remove nodes and add them to respective groups
			nodesToRemove.forEach(({ node, group }) => {
				scene.remove(node);
				group.add(node);
			});
