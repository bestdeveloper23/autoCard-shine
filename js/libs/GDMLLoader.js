...
class GDMLLoader extends Loader {
    ...
    parse(text, path) {
        ...
        function parseSolids() {
            ...
            for (var i = 0; i < solids.length; i++) {
                ...
                if (type === 'torus') {
                    ...
                    let aunit = solid.getAttribute('aunit');
                ...
                if (type === 'trap') {
                    ...
                    if (aunit === 'rad') {
                        pTheta *= 180.0 / Math.PI;
                        pPhi *= 180.0 / Math.PI;
                        pAlpha *= 180.0 / Math.PI;
                    }
                ...
                if (type === 'hype') {
                    ...
                    if (aunit === 'rad') {
                        stereo1 *= 180.0 / Math.PI;
                        stereo2 *= 180.0 / Math.PI;
                    }
        ...
        function getMesh(solid) {
            
            var name = '';
            solidsArray[name] = solid;

            var type = solid.nodeName;
            if (meshes[solid.getAttribute('name')]){
                return meshes[solid.getAttribute('name')];
            } else if (type === 'box') {
                ...
                var newMesh = new Mesh(geometry, material);
                ...
                if (defines[v1] && defines[v2] && defines[v3] && defines[v4]) {
                    ...
                    var newMesh = new Mesh(tet, material);
                    meshes[name] = newMesh;
                ...
                let first = solid.querySelectorAll('first')[0];
                let second = solid.querySelectorAll('second')[0];

                var firstName = first.getAttribute('ref');
                var secondName = second.getAttribute('ref');

                let firstPosition = new Vector3(0,0,0);
                let secondPosition = new Vector3(0,0,0);

                let fisrtPos = solid.getElementsByTagName('firstpositionref')[0];
                let secondPos = solid.getElementsByTagName('secondpositionref')[0];
                if (fisrtPos) {
                    firstPosition = defines[fisrtPos.getAttribute('ref')];

                } else if(solid.getElementsByTagName('firstposition')[0]) {
                    var x = solid.getElementsByTagName('firstposition')[0].getAttribute('x') || 0;
                    var y = solid.getElementsByTagName('firstposition')[0].getAttribute('y') || 0;
                    var z = solid.getElementsByTagName('firstposition')[0].getAttribute('z') || 0;

                    firstPosition.set(x,y,z);
                }

                if (secondPos) {
                    secondPosition = defines[secondPos.getAttribute('ref')];

                } else if(solid.getElementsByTagName('secondposition')[0]) {
                    var x = solid.getElementsByTagName('secondposition')[0].getAttribute('x') || 0;
                    var y = solid.getElementsByTagName('secondposition')[0].getAttribute('y') || 0;
                    var z = solid.getElementsByTagName('secondposition')[0].getAttribute('z') || 0;

                    secondPosition.set(x,y,z);
                }


                if (meshes[firstName]) {
                    firstchild = meshes[firstName];
                } else {
                    firstchild = getMesh(solidsArray[firstName]);
                    meshes[firstName] = firstchild;
                    console.log(firstchild, firstPosition);
                    firstchild.position.copy(firstPosition);
                }
                if (meshes[secondName]) {
                    secondchild = meshes[secondName];
                } else {
                    secondchild = getMesh(solidsArray[secondName]);
                    meshes[secondName] = secondchild;
                    console.log(secondchild, secondPosition);
                    secondchild.position.copy(secondPosition);
                }
                var mesh;
                ...
                mesh = CSG.toMesh(aCSG, new Matrix4());
                mesh.name = 'united_object'

                var position = new Vector3(0, 0, 0);
                var rotation = new Vector3(0, 0, 0);
                var posref = solid.getElementsByTagName('positionref')[0];
                var rotref = solid.getElementsByTagName('rotationref')[0];

                if(posref) {
                    position = defines[posref.getAttribute('ref')];
                } else if(solid.getElementsByTagName('position')[0]) {
                    var posx = solid.getElementsByTagName('position')[0].getAttribute('x') || 0;
                    var posy = solid.getElementsByTagName('position')[0].getAttribute('y') || 0;
                    var posz = solid.getElementsByTagName('position')[0].getAttribute('z') || 0;
                    position.set(posx, posy, posz);
                }

                if(rotref) {
                    rotation = defines[rotref.getAttribute('ref')];
                } else if(solid.getElementsByTagName('rotation')[0]) {
                    var rotx = solid.getElementsByTagName('rotation')[0].getAttribute('x') || 0;
                    var roty = solid.getElementsByTagName('rotation')[0].getAttribute('y') || 0;
                    var rotz = solid.getElementsByTagName('rotation')[0].getAttribute('z') || 0;
        ...
        function parsePhysVols() {
            ...
            for (var i = 0; i < physvols.length; i++) {
                ...
                for (var j = 0; j < children.length; j++) {
                    ...
                    if (type === 'volumeref') {
                        ...
                        solid = solidsArray[refs[volumeref]];
                ...
                if (geometry) {
                    ...
                    meshes[solid.getAttribute('name')] = mesh;
                    ...
                    meshes[solid.getAttribute('name')] = newMesh;
                    group.add(newMesh);
                }
            }
            ...
            for (var i = 0; i < physvols.length; i++) {
                ...
                if(solid) {
                    console.log(solid)
                    if (solid.nodeName === 'union' || solid.nodeName === 'subtraction' || solid.nodeName === 'intersection') {
                        var booleanMesh = getMesh(solid);
                        booleanMesh.visible = true;
                        booleanMesh.position.set(position.x, position.y, position.z);
                        booleanMesh.rotation.set(rotation.x, rotation.y, rotation.z);
                        meshes[solid.getAttribute('name')] = booleanMesh;

                        console.log(booleanMesh)
                        group.add(booleanMesh);
                    }
                }
